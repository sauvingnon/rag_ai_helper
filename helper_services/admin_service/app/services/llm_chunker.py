...
Разбей текст на смысловые чанки по следующим правилам:

ПРАВИЛА ОБЪЕДИНЕНИЯ (важно):
- Все поля, относящиеся к одному человеку (ФИО, должность, стаж, образование, квалификация и т.д.) — это ОДИН чанк.
- Все поля, описывающие один курс, дисциплину или программу — это ОДИН чанк.
- Один раздел документа (один абзац, одна статья, один пункт) — это ОДИН чанк.
- НЕ дели одну сущность на отдельные строки или поля.
- Минимальный чанк — не менее 2-3 смысловых предложений/фактов; одиночные строки объединяй с соседними.

КОГДА ДЕЛИТЬ:
- Разные люди — разные чанки.
- Разные разделы документа с разными темами — разные чанки.
- Большие разделы (>500 слов) с разными подтемами можно разделить.
...
def _extract_pdf(content: bytes) -> str:
    ...
    for i, page in enumerate(reader.pages):
        try:
            ...
            logger.warning("PDF страница %d пропущена: %s", i, e)
    return "\n".join(pages)
